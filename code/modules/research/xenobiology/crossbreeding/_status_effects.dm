...
/datum/status_effect/stabilized/purple/tick(seconds_between_ticks)
	...
	if(owner.getBruteLoss() > 0)
		need_mob_update += owner.adjustBruteLoss(-5, updating_health = FALSE)
		healed_last_tick = TRUE

	if(owner.getFireLoss() > 0)
		need_mob_update += owner.adjustFireLoss(-5, updating_health = FALSE)
		healed_last_tick = TRUE

	if(owner.getToxLoss() > 0)
		// Forced, so slimepeople are healed as well.
		need_mob_update += owner.adjustToxLoss(-5, updating_health = FALSE, forced = TRUE)
		healed_last_tick = TRUE
